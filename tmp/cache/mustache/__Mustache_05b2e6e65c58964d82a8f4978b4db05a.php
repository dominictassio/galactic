...
class __Mustache_05b2e6e65c58964d82a8f4978b4db05a extends Mustache_Template
{
...
';
        ...
        $buffer .= $this->section45a392bea3e8139b2ea8347e953de7fc($context, $indent, $value);
        $buffer .= $indent . '            </div>
';
        $buffer .= $indent . '        </div>
';
...
';
    ...
    private function section45a392bea3e8139b2ea8347e953de7fc(Mustache_Context $context, $indent, $value)
    {
        $buffer = '';
        if (!is_string($value) && is_callable($value)) {
            $source = '
                    <div class="col-xs-1">
                        <img class="img-responsive" src="https://minotar.net/helm/{{ name }}" alt="{{ name }}" data-toggle="tooltip" data-placement="bottom" title="{{ name }}">
                    </div>
                ';
            $result = call_user_func($value, $source, $this->lambdaHelper);
            if (strpos($result, '{{') === false) {
