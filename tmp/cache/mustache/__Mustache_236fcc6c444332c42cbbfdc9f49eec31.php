...
class __Mustache_236fcc6c444332c42cbbfdc9f49eec31 extends Mustache_Template
{
...
';
        ...
        $buffer .= $this->section7288d0be8b8238a9c7a885307bfbbeb8($context, $indent, $value);
        $buffer .= $indent . '            </div>
';
        $buffer .= $indent . '        </div>
';
...
';
    ...
    private function section7288d0be8b8238a9c7a885307bfbbeb8(Mustache_Context $context, $indent, $value)
    {
        $buffer = '';
        if (!is_string($value) && is_callable($value)) {
            $source = '
                    <div class="col-xs-1">
                        <img class="img-responsive" src="https://minotar.net/avatar/{{ name }}" alt="{{ name }}" data-toggle="tooltip" data-placement="bottom" title="{{ name }}">
                    </div>
                ';
            $result = call_user_func($value, $source, $this->lambdaHelper);
            if (strpos($result, '{{') === false) {
