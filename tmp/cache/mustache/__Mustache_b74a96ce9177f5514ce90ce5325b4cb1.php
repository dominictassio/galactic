...
class __Mustache_b74a96ce9177f5514ce90ce5325b4cb1 extends Mustache_Template
{
...
';
        ...
        $buffer .= $this->section51a880c7f0db1d6c16f36bdfcab4386f($context, $indent, $value);
        $buffer .= $indent . '            </div>
';
        $buffer .= $indent . '        </div>
';
...
';
    ...
    private function section51a880c7f0db1d6c16f36bdfcab4386f(Mustache_Context $context, $indent, $value)
    {
        $buffer = '';
        if (!is_string($value) && is_callable($value)) {
            $source = '
                    <div class="col-xs-1">
                        <img class="img-rounded img-responsive" src="https://minotar.net/avatar/{{ name }}" alt="{{ name }}">
                        <span>{{ name }}</span>
                    </div>
                ';
            $result = call_user_func($value, $source, $this->lambdaHelper);
